...
public class Editor<T extends CommandSender> {
     ...
     */
    private final List<HistorySelection> history;

    /**
     * A list so for redoing actions
     */
    private final List<HistorySelection> undos;
     ...
     */
    public void setBlocks(Selection selection, Pattern pattern) {
        if (checkLimit()) {
            BlockQueue queue = new BlockQueue(pattern, this);

            store(selection.toHistory());

            queue.build(selection);

            this.change += selection.getDimensions().getVolume();

            send("You successfully set " + selection.getDimensions().getVolume() + " blocks");
        }
    }
    ...
    public void setSlowBlocks(CuboidSelection cuboid, Pattern pattern, int perTick) {
        if (checkLimit()) {
            SlowBlockQueue queue = new SlowBlockQueue(pattern, this, perTick);
    ...
    public void copyCuboid(CuboidSelection cuboid, int x, int z) {
        if (checkLimit()) {
            if (x > 0 && z > 0) {
                Cuboid2DResizeQueue queue = new Cuboid2DResizeQueue(this, x, z);
    ...
    public void copyCuboid(CuboidSelection cuboid, int x, int y, int z) {
        if (checkLimit()) {
            if (x > 0 && y > 0 && z > 0) {
                Cuboid3DResizeQueue queue = new Cuboid3DResizeQueue(this, x, y, z);
    ...
    /**
     * The same as {@link #store(HistorySelection)} but for undos.
     *
     * @param   selection
     *          The {@link HistorySelection} to be added to the history.
     */
    public void storeUndo(HistorySelection selection) {
    ...
    /**
     * Gets the thread used for getting blocks
     *
     * @return the thread
     */
