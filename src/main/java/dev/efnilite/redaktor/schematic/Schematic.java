...
import dev.efnilite.redaktor.event.SchematicPasteEvent;
import dev.efnilite.redaktor.event.SchematicSaveEvent;
import dev.efnilite.redaktor.schematic.io.SchematicReader;
import dev.efnilite.redaktor.schematic.io.SchematicWriter;
...
public class Schematic {
     ...
     *
     * @param   type
     *          The type in which it should be saved.
     *
     * @param   options
     *          The options for saving.
     */
    public void save(String file, SchematicType type, SaveOptions... options) {
        if (selection != null) {
            switch (type) {
                case SCHEMATIC:
                    Bukkit.getPluginManager().callEvent(new SchematicSaveEvent(this, type, file));
                ...
                case STRUCTURE:
                    ...
                    StructureManager manager = Redaktor.getStructureManager();
                    manager.save(new File(file), new CuboidSelection(selection.getPos1(), selection.getPos2(), selection.getWorld()));
                    break;
            }
        } else {
    ...
    /**
     * Defaults the {@link SchematicType} to Schematic
     *
     * @see #save(String, SchematicType, SaveOptions...)
     */
    public void save(String file, SaveOptions... options) {
        save(file, SchematicType.SCHEMATIC, options);
    }
